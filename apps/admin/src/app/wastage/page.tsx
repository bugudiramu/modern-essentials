...
import { Trash2, AlertCircle, TrendingDown } from "lucide-react";
...
export default function WastagePage() {
  ...
  if (loading)
    return (
      <div className="p-8 font-headline italic">Loading Wastage Log...</div>
    );
      ...
      <div className="flex-1 p-6 space-y-10 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between">
          <p className="text-muted-foreground max-w-2xl">
            Complete audit trail for broken, expired, or rejected stock. Part of
            the{" "}
            <span className="font-bold text-primary italic">
              The Honest Essentials Ops Transparency Initiative
            </span>
            .
          </p>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-3">
          <div className="rounded-2xl bg-surface-container-low p-6 shadow-inner transition-all hover:bg-surface-container-high group">
            ...
            <div className="font-headline text-4xl font-bold text-red-700">
              {totalWastage} units
            </div>
            <p className="text-xs text-muted-foreground mt-2 font-medium tracking-wide">
              All-time units written off
            </p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-separate border-spacing-0">
                <thead className="bg-surface-container-low/50">
                  <tr>
                    <th className="px-6 py-5 text-left text-xs font-bold text-muted-foreground uppercase tracking-widest">
                      Date
                    </th>
                    <th className="px-6 py-5 text-left text-xs font-bold text-muted-foreground uppercase tracking-widest">
                      Product
                  ...
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-6">
                        ...
                        <div className="text-[10px] font-black font-mono text-muted-foreground uppercase tracking-wider mt-0.5">
                          {log.sku}
                        </div>
                        {log.inventoryBatchId && (
                          <div className="text-[10px] font-black font-mono text-primary mt-1 uppercase tracking-tighter">
                            Batch: {log.inventoryBatchId.slice(-8)}
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-6 font-black text-red-700">
                        -{log.qty}
                      </td>
                      <td className="px-6 py-6">
