...
export function ProductList({ products }: ProductListProps) {
  const { addItem, items, updateItem, removeItem, isLoading } = useCart();
        ...
        return (
          ...
          >
            ...
            >
              ...
              <Badge
                variant="secondary"
                className="absolute top-4 right-4 bg-surface/90 backdrop-blur-md px-3 py-1 rounded-full text-[9px] uppercase tracking-widest font-black text-primary shadow-sm border-none"
              >
            ...
            <CardContent className="p-6 flex flex-col flex-grow space-y-3">
              <div className="space-y-1.5">
                ...
                >
                  <Heading
                    variant="h3"
                    className="text-xl text-primary group-hover/title:text-secondary transition-colors line-clamp-1 font-bold"
                  >
                    {product.name}
                  </Heading>
                </Link>
                <Text
                  variant="small"
                  className="text-primary/60 line-clamp-2 leading-relaxed text-xs"
                >
                  {product.description ||
                    "Farm fresh directly to your doorstep. Perfect for daily consumption, packed with care."}
                </Text>
              </div>
              ...
              <div className="mt-auto pt-4 flex items-center justify-between border-t border-primary/5">
                <div className="flex flex-col">
                  ...
                  {defaultVariant.subPrice && (
                    <Text
                      variant="xs"
                      className="text-secondary font-black text-[9px] uppercase tracking-tight mt-0.5"
                    >
                      Save ₹
                      {(
                        (defaultVariant.price - defaultVariant.subPrice) /
                        100
                      ).toFixed(2)}{" "}
                      with sub
                    </Text>
                ...
                {inCartQty > 0 ? (
                  ...
                  <Button
                    onClick={() =>
                      addItem(defaultVariant, 1, false, "WEEKLY", product)
                    }
                    isLoading={isLoading}
